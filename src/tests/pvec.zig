...
test "iterator" {
    ...
    for (data_sizes) |s| {
        ...
        var i: usize = 0;
        while (iter.nextPtr()) |v| : (i += 1) {
            try std.testing.expect(v.* == data[i]);
        }
    }
}

test "multi vec backend" {
    var gpa = std.heap.DebugAllocator(.{}){};
    defer {
        const deinit_status = gpa.deinit();
        if (deinit_status == .leak) std.debug.print("WARNING: memory leaked\n", .{});
...
test "multi iter field slice ptr" {
    var gpa = std.heap.DebugAllocator(.{}){};
    defer {
        const deinit_status = gpa.deinit();
        if (deinit_status == .leak) std.debug.print("WARNING: memory leaked\n", .{});
    ...
    for (data_sizes) |s| {
        ...
        var iter = vector.fieldsIterator(&.{ .field1, .field2 });
        for (0..data.len) |i| {
            const f1, const f2 = iter.nextPtr().?;
            try std.testing.expectEqual(data[i].field1, f1.*);
            try std.testing.expectEqual(data[i].field2, f2.*);
        }
        ...
        var tail_iter = vector.fieldsIterator(&.{ .field1, .field2 }).tail();
        for (1..data.len) |i| {
            const f1, const f2 = tail_iter.nextPtr().?;
            try std.testing.expectEqual(data[i].field1, f1.*);
            try std.testing.expectEqual(data[i].field2, f2.*);
        }
